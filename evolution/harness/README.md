...
labelled **`mutation`** (or comments on one), the loop runs the `/mutate` cycle through the Claude Code CLI
...
```
issue labelled `mutation` ──► GitHub Action ──► Claude Code runs /mutate
                                                       │
              scripts/mutate-ide.sh setup  ◄───────────┤  (fetch tickets+comments, branch)
              implement + test (dotnet/vite) ◄──────────┤  (smallest correct change + test)
              scripts/mutate-ide.sh finish ◄────────────┘  (push branch, open PR, reply on ticket)
```

- **Brain:** Claude Code CLI (your existing login) executing `.claude/commands/mutate.md`.
- **Infrastructure:** `scripts/mutate-ide.sh` (GitHub + git + build/test + PR). PR-only is enforced
...
./scripts/mutate-claude.sh --loop 30m  # tick every 30 min
```

Each tick runs `claude -p "/mutate"`. An **internal timer** inside `scripts/mutate-ide.sh` decides
...
`.github/workflows/mutation.yml` lets a ticket trigger evolution remotely. When an issue is labelled
`evolve` (or a tester comments on a labelled one), the workflow installs the Claude CLI and runs
**`scripts/mutate-claude.sh`** — the exact entrypoint the local timer uses. So CI and local share one
...
1. **PR-only.** `mutate-ide.sh` pushes a session branch and opens a PR. It `die`s if asked to act on
...
4. **Scope-limited.** The `/mutate` command instructs minimal, surgical edits and forbids touching
...
License (see `NOTICE`). The `/mutate` pipeline shape and the `plan`/`debug`/`test`/`self-assess`
