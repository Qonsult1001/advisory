...
public class LlmGatewayController : ControllerBase
{
    ...
    // Transparent proxy: forward EVERY caller header upstream EXCEPT the hop-by-hop / host-specific ones
    // below, which must be regenerated by our HttpClient for the new connection. Forwarding everything is
    // required for OAuth (subscription) tokens: Claude Code's Max/Pro OAuth token is validated by Anthropic
    // together with the client-identifying headers the tool sends (user-agent, x-app, x-stainless-*,
    // anthropic-dangerous-direct-browser-access, …). Stripping those to a fixed allowlist made Anthropic
    // reject the token as "OAuth access token is invalid" even though the token itself was valid. An x-api-key
    // caller is unaffected — its key isn't bound to those headers.
    private static readonly HashSet<string> DropHeaders = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        "Transfer-Encoding", "Upgrade", "TE", "Trailer", "Expect", "Content-Type",  // Content-Type set on the StringContent
        // strip inbound hop headers our reverse proxy / Kestrel added that shouldn't reach the origin
        "X-Forwarded-For", "X-Forwarded-Proto", "X-Forwarded-Host", "X-Real-IP",
        // DROP Accept-Encoding so the upstream returns an UNCOMPRESSED body. We relay the response bytes
        // verbatim (streaming path especially), so if the client asked for gzip and the upstream compressed
        // the reply, we'd hand the client compressed bytes it can't decode → "API Error: 400 <binary garbage>".
        // Forcing identity encoding keeps the relay a clean byte copy in both the streaming and buffered paths.
        "Accept-Encoding",
    };
    ...
    private async Task<IActionResult> Proxy(string? forcedProvider, string path, CancellationToken ct)
    {
        ...
        // Forward every inbound header except the hop-by-hop / host ones — a transparent proxy must not
        // reshape the caller's headers, or an OAuth token bound to them is rejected upstream.
        foreach (var h in Request.Headers)
        {
            if (DropHeaders.Contains(h.Key)) continue;
            req.Headers.TryAddWithoutValidation(h.Key, (string[])h.Value!);
        }
        ...
        try
        {
            ...
            if (wantsStream)
            {
                ...
                // On an upstream ERROR the SSE body is a JSON error, not a token stream — buffer it, record
                // the reason (so operators can see WHY the provider rejected), and return it as plain JSON so
                // the client shows the real message instead of a truncated/garbled stream.
                if (!upResp.IsSuccessStatusCode)
                {
                    var errBody = await upResp.Content.ReadAsStringAsync(ct);
                    rec.Reason = $"upstream {(int)upResp.StatusCode}: {errBody}";
                    _audit.Add(rec);
                    Response.StatusCode = (int)upResp.StatusCode;
                    Response.ContentType = upResp.Content.Headers.ContentType?.ToString() ?? "application/json";
                    await Response.WriteAsync(errBody, ct);
