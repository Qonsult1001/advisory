...
namespace PkgFirewall.Api.Controllers;
...
{
    private readonly PkgFirewall.Api.Catalog.CatalogService _catalog;
    public CatalogController(PkgFirewall.Api.Catalog.CatalogService catalog) => _catalog = catalog;
...
{
    private readonly IEnumerable<PkgFirewall.Api.Resolve.IDependencyResolver> _resolvers;
    public SbomController(IEnumerable<PkgFirewall.Api.Resolve.IDependencyResolver> r) => _resolvers = r;
    ...
    public async Task<ActionResult> Generate([FromBody] PackageRef pkg, CancellationToken ct)
    {
        ...
        var tree = resolver is not null
            ...
            : new List<PkgFirewall.Api.Resolve.DepNode> { new(pkg, 0, null) };
        var sbom = PkgFirewall.Api.Scan.SbomGenerator.CycloneDx(pkg, tree);
...
public class ScansController : ControllerBase
{
    ...
    private readonly PkgFirewall.Api.Scan.ScanStore _scans;
    public ScansController(INexusClient nexus, PkgFirewall.Api.Scan.ScanStore scans) { _nexus = nexus; _scans = scans; }
...
public class ReportsController : ControllerBase
{
    private readonly IAuditLog _audit;
    private readonly IPolicyStore _policy;
    private readonly PkgFirewall.Api.Catalog.OpRiskService _opRisk;
    public ReportsController(IAuditLog audit, IPolicyStore policy, PkgFirewall.Api.Catalog.OpRiskService opRisk)
...
public class AppTrustController : ControllerBase
{
    private readonly IPolicyStore _policy;
    private readonly IAuditLog _audit;
    private readonly PkgFirewall.Api.Auth.ICurrentUser _user;
    public AppTrustController(IPolicyStore policy, IAuditLog audit, PkgFirewall.Api.Auth.ICurrentUser user)
...
{
    private readonly PkgFirewall.Api.Catalog.AiCatalogService _svc;
    private readonly PkgFirewall.Api.Catalog.WeightVerifier _verifier;
    private readonly PkgFirewall.Api.Catalog.VerificationJobService _jobs;
    private readonly PkgFirewall.Api.Catalog.ConsumedModelStore _consumed;
    private readonly IPolicyStore _policy;
    private readonly PkgFirewall.Api.Auth.ICurrentUser _user;
    public AiCatalogController(PkgFirewall.Api.Catalog.AiCatalogService svc, PkgFirewall.Api.Catalog.WeightVerifier verifier,
        PkgFirewall.Api.Catalog.VerificationJobService jobs, PkgFirewall.Api.Catalog.ConsumedModelStore consumed,
        IPolicyStore policy, PkgFirewall.Api.Auth.ICurrentUser user)
...
{
    private readonly PkgFirewall.Api.Scan.OnDemandScanService _svc;
    public OnDemandController(PkgFirewall.Api.Scan.OnDemandScanService svc) => _svc = svc;
...
public class AiController : ControllerBase
{
    ...
    public AiController(IGroqClient groq, IPolicyStore policy, IAuditLog audit, PkgFirewall.Api.Auth.ICurrentUser user)
