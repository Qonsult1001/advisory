...
- **Approval banner** - *"[OK] Approved for downloading"* (green) or *"[blocked] Blocked by policy"* (red) - the
...
The profile matching your current policy shows **[OK] ACTIVE**.
...
tells you exactly what it changed (with a **see it v** link to each affected control). Click a **Try
 ...
 **Stage** ([OK] Approved / ⏸ Held / ⛔ Revoked). **Click a package** to drill into only that package's
